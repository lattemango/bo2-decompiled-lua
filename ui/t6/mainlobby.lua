...
CoD.MainLobby.ShouldPreventCreateLobby = function()
    if UIExpression.AcceptingInvite() == 1 or Engine.IsJoiningAnotherParty() == 1 or UIExpression.PrivatePartyHost() ==
        0 or Engine.IsGameLobbyRunning() then
        return true
    else
        return false
    end
...
CoD.MainLobby.OnlinePlayAvailable = function(MainLobbyWidget, ClientInstance, Boolean)
    ...
    elseif (UIExpression.IsContentRatingAllowed(ClientInstance.controller) == 0 or
        UIExpression.IsAnyControllerMPRestricted() == 1) and not Boolean then
        local ErrorPopup = MainLobbyWidget:openPopup("Error", ClientInstance.controller)
...
CoD.MainLobby.OpenPlayerMatchPartyLobby = function(MainLobbyWidget, ClientInstance)
    ...
    elseif CoD.MainLobby.OnlinePlayAvailable(MainLobbyWidget, ClientInstance) == 1 then
        ...
        local GivenProbation, LocalClientIndexGivenProbation =
            Engine.ProbationCheckForProbation(CoD.GAMEMODE_PUBLIC_MATCH)
        ...
        local MaxLocalPlayers =
            UIExpression.DvarInt(LocalClientIndexGivenProbation, "party_maxlocalplayers_playermatch")
...
CoD.MainLobby.OpenLeagueSelectionPopup = function(MainLobbyWidget, ClientInstance)
    ...
    elseif CoD.MainLobby.OnlinePlayAvailable(MainLobbyWidget, ClientInstance) == 1 then
        ...
        local GivenProbation, LocalClientIndexGivenProbation =
            Engine.ProbationCheckForProbation(CoD.GAMEMODE_LEAGUE_MATCH)
        if GivenProbation == true then
...
CoD.MainLobby.OpenCustomGamesLobby = function(MainLobbyWidget, ClientInstance)
    if CoD.MainLobby.ShouldPreventCreateLobby() then
        return
    elseif CoD.MainLobby.OnlinePlayAvailable(MainLobbyWidget, ClientInstance) == 1 and
        CoD.MainLobby.IsControllerCountValid(MainLobbyWidget, ClientInstance.controller, UIExpression.DvarInt(
            ClientInstance.controller, "party_maxlocalplayers_privatematch")) == 1 then
...
CoD.MainLobby.OpenTheaterLobby = function(MainLobbyWidget, ClientInstance)
    if CoD.MainLobby.ShouldPreventCreateLobby() then
        return
    elseif UIExpression.CanSwitchToLobby(ClientInstance.controller, Dvar.party_maxplayers_theater:get(),
        Dvar.party_maxlocalplayers_theater:get()) == 0 then
    ...
    elseif CoD.MainLobby.OnlinePlayAvailable(MainLobbyWidget, ClientInstance) == 1 and
        CoD.MainLobby.IsControllerCountValid(MainLobbyWidget, ClientInstance.controller, UIExpression.DvarInt(
            ClientInstance.controller, "party_maxlocalplayers_theater")) == 1 then
...
CoD.MainLobby.OpenCODTV = function(MainLobbyWidget, ClientInstance)
    ...
    elseif Engine.IsLivestreamEnabled() then
        ...
        return
    elseif CoD.MainLobby.OnlinePlayAvailable(MainLobbyWidget, ClientInstance) == 1 and Engine.IsCodtvContentLoaded() ==
        true then
...
CoD.MainLobby.OpenStore = function(MainLobbyWidget, ClientInstance)
    ...
    if Engine.CheckNetConnection() == false then
        local NetConnectionCheckPopup = MainLobbyWidget:openPopup("popup_net_connection_store",
...
CoD.MainLobby.PopulateButtons_Multiplayer = function(MainLobbyButtonPane)
    MainLobbyButtonPane.body.serverBrowserButton = MainLobbyButtonPane.body.buttonList:addButton(Engine.Localize(
        "SERVER BROWSER"), nil, 1)
    MainLobbyButtonPane.body.serverBrowserButton.hintText = Engine.Localize(
        CoD.MPZM("MPUI_PLAYER_MATCH_DESC", "ZMUI_PLAYER_MATCH_DESC"))
    ...
    if Engine.IsBetaBuild() then
        MainLobbyButtonPane.body.matchmakingButton = MainLobbyButtonPane.body.buttonList:addButton(Engine.Localize(
            "MENU_MATCHMAKING_CAPS"), nil, 3)
    ...
    MainLobbyButtonPane.body.matchmakingButton.hintText = Engine.Localize(
        CoD.MPZM("MPUI_PLAYER_MATCH_DESC", "ZMUI_PLAYER_MATCH_DESC"))
    ...
    if not Engine.IsBetaBuild() then
        MainLobbyButtonPane.body.customGamesButton = MainLobbyButtonPane.body.buttonList:addButton(Engine.Localize(
            "MENU_CUSTOMGAMES_CAPS"), nil, 4)
        MainLobbyButtonPane.body.customGamesButton.hintText = Engine.Localize(
            CoD.MPZM("MPUI_CUSTOM_MATCH_DESC", "ZMUI_CUSTOM_MATCH_DESC"))
    ...
    MainLobbyButtonPane.body.theaterButton = MainLobbyButtonPane.body.buttonList:addButton(Engine.Localize(
        "MENU_THEATER_CAPS"), nil, 5)
    MainLobbyButtonPane.body.theaterButton:setActionEventName("open_theater_lobby")
    MainLobbyButtonPane.body.theaterButton.hintText =
        Engine.Localize(CoD.MPZM("MPUI_THEATER_DESC", "ZMUI_THEATER_DESC"))
    MainLobbyButtonPane.body.postTheaterSpacer = MainLobbyButtonPane.body.buttonList:addSpacer(
        CoD.CoD9Button.Height / 2, 6)
    if not Engine.IsBetaBuild() then
        MainLobbyButtonPane.body.barracksButton = MainLobbyButtonPane.body.buttonList:addButton(Engine.Localize(
            "MENU_BARRACKS_CAPS"), nil, 7)
        MainLobbyButtonPane.body.barracksButton.id = "CoD9Button" .. "." .. "MainLobby" .. "." ..
                                                         Engine.Localize("MENU_BARRACKS_CAPS")
    ...
    if CoD.isZombie == false and not Engine.IsBetaBuild() and (CoD.isXBOX or CoD.isPS3) and Engine.IsEliteAvailable() and
        Engine.IsEliteButtonAvailable() then
        MainLobbyButtonPane.body.eliteAppButton = MainLobbyButtonPane.body.buttonList:addButton(Engine.Localize(
            "MENU_ELITE_CAPS"), nil, 8)
    ...
    MainLobbyButtonPane.body.optionsButton = MainLobbyButtonPane.body.buttonList:addButton(Engine.Localize(
        "MENU_OPTIONS_CAPS"), nil, 11)
...
CoD.MainLobby.PopulateButtons_Zombie = function(MainLobbyButtonPane)
    ...
    -- MainLobbyButtonPane.body.matchmakingButton:setActionEventName("open_player_match_party_lobby")
    -- CoD.SetupMatchmakingLock(MainLobbyButtonPane.body.matchmakingButton)
    MainLobbyButtonPane.body.soloPlayButton = MainLobbyButtonPane.body.buttonList:addButton(Engine.Localize(
        "ZMUI_SOLO_PLAY_CAPS"), nil, 3)
...
CoD.MainLobby.PopulateButtons = function(MainLobbyButtonPane)
    ...
    if CoD.isWIIU then
        MainLobbyButtonPane.body.controlsButton = MainLobbyButtonPane.body.buttonList:addButton(Engine.Localize(
            "MENU_CONTROLLER_SETTINGS_CAPS"), nil, 9)
    ...
    if CoD.isOnlineGame() then
        ...
        if PlayerCountText ~= "" then
            ...
            PlayerCountUpdateTimer = LUI.UITimer.new(60000, "update_online_player_count", false,
                MainLobbyButtonPane.playerCountLabel)
        else
            PlayerCountUpdateTimer = LUI.UITimer.new(1000, "update_online_player_count", false,
                MainLobbyButtonPane.playerCountLabel)
        end
        MainLobbyButtonPane.playerCountLabel:registerEventHandler("update_online_player_count",
            CoD.MainLobby.UpdateOnlinePlayerCount)
...
CoD.MainLobby.PlayerSelected = function(MainLobbyWidget, PlayerSelectedEvent)
    if MainLobbyWidget.joinable ~= nil and
        CoD.canJoinSession(UIExpression.GetPrimaryController(), PlayerSelectedEvent.playerXuid) then
...
CoD.MainLobby.BusyList_Update = function(f28_arg0, f28_arg1, f28_arg2, f28_arg3, f28_arg4)
    CoD.PlayerList.Update(f28_arg0, Engine.GetBusyFriendsOfAllLocalPlayers(f28_arg0.maxRows - f28_arg2), f28_arg2,
        f28_arg3, f28_arg4)
end
...
CoD.MainLobby.AddLobbyPaneElements = function(LobbyPane, MenuParty)
    CoD.LobbyPanes.addLobbyPaneElements(LobbyPane, MenuParty,
        UIExpression.DvarInt(nil, "party_maxlocalplayers_mainlobby"))
