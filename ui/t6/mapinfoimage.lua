...
CoD.MapInfoImage.new = function ( f1_arg0 )
	...
	self:registerEventHandler( "gamelobby_update", CoD.MapInfoImage.RefreshEvent )
	self:registerEventHandler( "game_options_update", CoD.MapInfoImage.RefreshEvent )
	self:registerEventHandler( "update_livestream_camera", CoD.MapInfoImage.UpdateLiveStreamCamera )
	self.id = "MapInfoImage"
	self.mapTextSize = CoD.textSize.Condensed
	self.gametypeTextSize = CoD.textSize.Default
	self.loadingTextSize = CoD.textSize.Default
	self.unselectedFilmTextSize = CoD.textSize.Default
	local f1_local1 = CoD.MapInfoImage.MapImageLeft
	local f1_local2 = CoD.MapInfoImage.MapImageBottom
	self.mapImage = LUI.UIImage.new( {
		leftAnchor = true,
		rightAnchor = false,
		left = f1_local1,
	...
	self.unselectedFilmImageBackground = LUI.UIImage.new( {
		leftAnchor = true,
		rightAnchor = false,
		left = f1_local1,
		right = f1_local1 + CoD.MapInfoImage.MapImageWidth,
	...
	self:addElement( self.unselectedFilmImageBackground )
	self.unselectedFilmImage = LUI.UIImage.new( {
		leftAnchor = true,
		rightAnchor = false,
		left = f1_local1 + 70,
	...
	self:addElement( self.unselectedFilmImage )
	local f1_local5 = CoD.MPZM( -10, -20 )
	local f1_local6 = CoD.MPZM( -2, -14 )
	self.gameTypeText = LUI.UIText.new( {
		leftAnchor = true,
		rightAnchor = false,
	...
	self:addElement( self.gameTypeText )
	self.mapNameText = LUI.UIText.new( {
		leftAnchor = true,
		rightAnchor = false,
		left = f1_local1 + CoD.MapInfoImage.MapImageWidth - 1000,
		right = f1_local1 + CoD.MapInfoImage.MapImageWidth + f1_local5,
	...
	self:addElement( self.mapNameText )
	local f1_local7 = UIExpression.ToUpper( nil, Engine.Localize( "EXE_LOADING" ) )
	local f1_local8 = {}
	f1_local8 = GetTextDimensions( f1_local7, CoD.fonts.Default, self.loadingTextSize )
	local f1_local9 = f1_local8[3]
	self.loadingText = LUI.UIText.new()
	self.loadingText:setLeftRight( false, false, -f1_local9 / 2 + CoD.MapInfoImage.SpinnerImageXPadding, f1_local9 / 2 + CoD.MapInfoImage.SpinnerImageXPadding )
	self.loadingText:setTopBottom( false, false, CoD.MapInfoImage.SpinnerImageDimension + CoD.MapInfoImage.SpinnerImageYPadding, CoD.MapInfoImage.SpinnerImageDimension + CoD.MapInfoImage.SpinnerImageYPadding + self.loadingTextSize )
	self.loadingText:setRGB( CoD.BOIIOrange.r, CoD.BOIIOrange.g, CoD.BOIIOrange.b )
	self.loadingText:setAlpha( 0 )
	self.loadingText:setText( f1_local7 )
	self:addElement( self.loadingText )
	self.unselectedFilmText = LUI.UIText.new( {
		leftAnchor = true,
		rightAnchor = false,
		left = f1_local1 + 30,
		...
		topAnchor = true,
		bottomAnchor = false,
		top = CoD.MapInfoImage.MapImageHeight / 2 - self.unselectedFilmTextSize / 2,
		bottom = CoD.MapInfoImage.MapImageHeight / 2 + self.unselectedFilmTextSize / 2,
		font = CoD.fonts.Default,
		alignment = LUI.Alignment.Center
	} )
	self.unselectedFilmText:setAlpha( 0 )
	self:addElement( self.unselectedFilmText )
	local f1_local10 = f1_local2 + f1_local3 + f1_local6 - 45
	self.modifedCustomGameElement = LUI.UIElement.new()
	self.modifedCustomGameElement:setLeftRight( true, false, f1_local1 + 2, f1_local1 + CoD.MapInfoImage.MapImageWidth - 2 )
	self.modifedCustomGameElement:setTopBottom( false, true, f1_local10 - 32, f1_local10 )
	self:addElement( self.modifedCustomGameElement )
	...
	self.modifedCustomGameElement:addElement( f1_local11 )
	...
	self.modifedCustomGameElement:addElement( f1_local12 )
	...
	self.modifedCustomGameElement.text = f1_local13
	self.modifedCustomGameElement:addElement( f1_local13 )
	self.modifedCustomGameElement:setAlpha( 0 )
	self.livestreamCam = LUI.UIImage.new( {
		leftAnchor = true,
		rightAnchor = false,
		left = f1_local1,
		right = f1_local1 + CoD.MapInfoImage.MapImageWidth,
	...
	self:addElement( self.livestreamCam )
	CoD.MapInfoImage.UpdateLiveStreamCamera( self )
	self.update = CoD.MapInfoImage.Update
	if CoD.isZombie == true then
		self.update = CoD.MapInfoImage.ZombieUpdate
	end
	self.show = CoD.MapInfoImage.Show
	self.hide = CoD.MapInfoImage.Hide
	self.setModifiedCustomGame = CoD.MapInfoImage.SetModifedCustomGame
	return self
end
