...
CoD.Menu.addFriendsButton = function(f12_arg0)
    f12_arg0.friendsButton = CoD.ButtonPrompt.new("alt2", Engine.Localize("MENU_FRIENDS"), f12_arg0,
        "button_prompt_friends", false, false, false, false, "F")
    f12_arg0:addRightButtonPrompt(f12_arg0.friendsButton)
end

CoD.Menu.addFriendsButtonSPMainMenu = function(f13_arg0)
    f13_arg0.friendsButton = CoD.ButtonPrompt.new("start", Engine.Localize("MENU_FRIENDS"), f13_arg0,
        "button_prompt_friends", false, false, false, false, "F")
    f13_arg0:addRightButtonPrompt(f13_arg0.friendsButton)
end

CoD.Menu.addJoinButton = function(f14_arg0)
    f14_arg0.joinButton = CoD.ButtonPrompt.new("alt1", Engine.Localize("MENU_JOIN_GAME"), f14_arg0,
        "button_prompt_join", false, false, false, false, "J")
...
CoD.Menu.addPartyPrivacyButton = function(f16_arg0)
    ...
    if f16_local0 then
        f16_arg0.partyPrivacyButton = CoD.ButtonPrompt.new("alt1", Engine.Localize("MPUI_LOBBY_PRIVACY"), f16_arg0,
            "button_prompt_partyprivacy", false, false, false, false, "P")
...
CoD.Menu.addGlassesToggleButton = function(f25_arg0)
    f25_arg0.glassesToggleButton = CoD.ButtonPrompt.new("right_trigger", Engine.Localize("MENU_TURN_OFF_HUD"), nil, nil,
...
CoD.Menu.addGlassesToggleOnButton = function(f26_arg0)
    f26_arg0.glassesToggleButton = CoD.ButtonPrompt.new("right_trigger", Engine.Localize("MENU_TURN_ON_HUD"), nil, nil,
        false, false, false, "space")
    f26_arg0:addRightButtonPrompt(f26_arg0.glassesToggleButton)
end

CoD.Menu.addPrestigeUnlockButton = function(f27_arg0)
    f27_arg0.prestigeUnlockButton = CoD.ButtonPrompt.new("shoulderr", Engine.Localize("MPUI_PERMANENT_UNLOCKS_OPTION"),
        f27_arg0, "prestige_unlock", false, false, false, false, "D")
...
CoD.Menu.addLargePopupBackground = function(f30_arg0)
    if CoD.isZombie == true then
        ...
        f30_arg0.topFrame:setTopBottom(false, false, -f30_local2 / 2 - f30_local0 - f30_local3,
            -f30_local2 / 2 - f30_local0)
        ...
        f30_arg0.bottomFrame:setTopBottom(false, false, f30_local2 / 2 - f30_local0 + f30_local3,
            f30_local2 / 2 - f30_local0)
...
CoD.Menu.addMediumPopupBackground = function(f31_arg0)
    ...
    if CoD.isZombie == true then
        ...
        f31_arg0.topFrame:setTopBottom(false, false, -f31_local2 / 2 - f31_local3 - f31_local4,
            -f31_local2 / 2 - f31_local3)
        ...
        f31_arg0.bottomFrame:setTopBottom(false, false, f31_local2 / 2 - f31_local3 + f31_local4,
            f31_local2 / 2 - f31_local3)
...
CoD.Menu.goBack = function(menuPane, controller)
    if menuPane.backOutSFX ~= nil then
        Engine.PlaySound(menuPane.backOutSFX)
    end

    menuPane:returnToPreviousMenu(controller)
end
...
CoD.Menu.GamepadButton = function(menuWidget, client)
    if menuWidget.m_inputDisabled then
        return
    elseif menuWidget.anyControllerAllowed == true or UIExpression.IsControllerBeingUsed(client.controller) == 1 or
        menuWidget.unusedControllerAllowed == true then
        if menuWidget.anyControllerAllowed == true or menuWidget.m_ownerController == nil or
            menuWidget.m_ownerController == client.controller then
            return menuWidget:dispatchEventToChildren(client)
        else

        end
    else
        local tempName = client.name
        client.name = "unused_gamepad_button"
        local f38_local1 = menuWidget:dispatchEventToChildren(client)
