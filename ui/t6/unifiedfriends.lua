...
CoD.FriendsListPopup.JoinSession = function(f6_arg0, f6_arg1)
    if UIExpression.IsInGame() == 1 and CoD.isZombie == false then
        if Engine.GameModeIsMode(CoD.GAMEMODE_LEAGUE_MATCH) and
            Engine.ProbationCheckForQuitWarning(CoD.GAMEMODE_LEAGUE_MATCH) then
            f6_arg0:openPopup("popup_probation_join_quit_warning", f6_arg1.controller)
            return
        elseif Engine.GameModeIsMode(CoD.GAMEMODE_PUBLIC_MATCH) and
            Engine.ProbationCheckForQuitWarning(CoD.GAMEMODE_PUBLIC_MATCH) then
            f6_arg0:openPopup("popup_probation_join_quit_warning", f6_arg1.controller)
            return
        end
    end
...
CoD.FriendsListPopup.FriendFocusChanged = function(f9_arg0)
    ...
    if f9_local0 ~= nil and f9_local0.playerXuid ~= nil and CoD.FriendDetails ~= nil then
        ...
        if f9_local1 ~= CoD.playerListType.friendRequest then
            CoD.FriendDetails.refresh(f9_arg0.details, f9_arg0.m_ownerController, f9_local0.playerXuid,
                f9_local0.playerIndex, f9_local1)
        end
    end
end
...
CoD.FriendsListPopup.UpdatePromptButtonVis = function(f10_arg0)
    ...
    if CoD.FriendsListPopup.Mode == CoD.playerListType.facebook and nil ~= Engine.IsFacebookLinked and
        Engine.IsFacebookLinked(f10_local0) == false and Engine.IsPlayerUnderage(f10_local0) == false and 0 ==
        UIExpression.IsInGame() then
...
CoD.FriendsListPopup.RefreshList = function(f13_arg0, f13_arg1)
    ...
    if f13_arg0 ~= nil and f13_arg0.listBox ~= nil then
        ...
        f13_arg0.listBox:setTotalItems(Engine.GetPlayerCount(f13_arg0.m_ownerController, CoD.FriendsListPopup.Mode),
            f13_local1.index)
        CoD.FriendsListPopup.UpdatePromptButtonVis(f13_arg0)
        f13_arg0.listBox:refresh()
        if Engine.GetPlayerCount(f13_arg0.m_ownerController, CoD.FriendsListPopup.Mode) == 0 and CoD.FriendDetails ~=
            nil then
...
LUI.createMenu.FriendsList = function(f28_arg0, f28_arg1)
    ...
    f28_local0.selectButton = CoD.ButtonPrompt.new("primary", Engine.Localize("MENU_SELECT"), f28_local0,
        "friend_selected")
    local f28_local1
    ...
    if f28_local1 == true then
        f28_local0.inviteButton = CoD.ButtonPrompt.new("select", Engine.Localize("MENU_INVITE_GAME"), f28_local0,
            "friend_invitetosession", false, false, false, false, "I")
        f28_local0.joinButton = CoD.ButtonPrompt.new("alt1", Engine.Localize("MENU_JOIN_IN_PROGRESS"), f28_local0,
            "friend_joinsession", false, false, false, false, "J")
    end
    f28_local0.backButton = CoD.ButtonPrompt.new("secondary", Engine.Localize("MPUI_DONE"), f28_local0,
        "close_friends_list")
    f28_local0.partyButton = CoD.ButtonPrompt.new("start", Engine.Localize("MENU_XBOXLIVE_PARTY"), f28_local0,
        "xboxlive_party")
    f28_local0.linkFBButton = CoD.ButtonPrompt.new("alt1", Engine.Localize("MENU_FB_LINK"), f28_local0,
        "fb_linkFacebook")
    if CoD.isPS3 then
        f28_local0.invitationsButton = CoD.ButtonPrompt.new("start", Engine.Localize("PLATFORM_VIEW_GAME_INVITES"),
            f28_local0, "ps3_invitations")
    end
    if not CoD.isSinglePlayer then
...
LUI.createMenu.IngameFriendsList = function(f29_arg0)
    ...
    f29_local0.selectButton = CoD.ButtonPrompt.new("primary", Engine.Localize("MENU_SELECT"), f29_local0,
        "friend_selected")
    f29_local0.inviteButton = CoD.ButtonPrompt.new("alt2", Engine.Localize("MENU_INVITE_GAME"), f29_local0,
        "friend_invitetosession", false, false, false, false, "I")
    f29_local0.joinButton = CoD.ButtonPrompt.new("alt1", Engine.Localize("MENU_JOIN_IN_PROGRESS"), f29_local0,
        "friend_joinsession", false, false, false, false, "J")
    f29_local0.backButton = CoD.ButtonPrompt.new("secondary", Engine.Localize("MPUI_DONE"), f29_local0,
        "button_prompt_back")
    if not CoD.isSinglePlayer then
