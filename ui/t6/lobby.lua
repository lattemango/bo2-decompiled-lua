...
CoD.Lobby.New = function(f15_arg0, f15_arg1, f15_arg2, f15_arg3, f15_arg4, f15_arg5)
    ...
    if CoD.isZombie == true then
        ...
        f15_local0:registerEventHandler("elite_registration_email_popup_requested",
            CoD.EliteRegistrationEmailPopup.EliteRegistrationEmailPopupRequested)
    ...
    f15_local0.panelManager = CoD.PanelManager.New(UIExpression.GetPrimaryController(), f15_local0.width,
        f15_local0.height)
    ...
    f15_local0.lobbyPane:addPanelElements(f15_arg4, UIExpression.DvarInt(f15_arg1, "party_maxlocalplayers"), f15_arg2,
        f15_arg5)
    ...
    if not CoD.isSinglePlayer == true and Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == false and
        UIExpression.SessionMode_IsSystemlinkGame() == 0 then
    ...
    if ShowGlobe then
        if CoD.isZombie == true then
            if Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == false and
                UIExpression.SessionMode_IsSystemlinkGame() ~= 1 then
                ShowGlobe()
            end
        else
            ShowGlobe()
...
CoD.Lobby.OpenMenu = function(f20_arg0, f20_arg1)
    if f20_arg0.menuName == f20_arg1.menuName then
        return
    elseif CoD.isZombie == true and f20_arg1.menuName == "MainMenu" and f20_arg0.previousMenuName == "MainMenu" and
        f20_arg0.menuName == "PrivateLocalGameLobby" then
    ...
    if CoD.isZombie == true and f20_arg1.menuName == "MainLobby" and CoD.Zombie.OpenMenuSelfMenuNames[f20_arg0.menuName] ==
        1 then
...
--- Go back?
--- @param lobbyWidget any
--- @param client any
CoD.Lobby.GoBack = function(lobbyWidget, client)
    if lobbyWidget.buttonPane.body ~= nil then
        lobbyWidget.buttonPane.body.buttonList:saveState()
    end

    -- Button pane is the buttons on the left (eg Create a Class, Barracks) from my understanding.
    lobbyWidget.buttonPane:animateToState("left_offscreen", lobbyWidget.fadeOutTime, true, true)
    -- Lobby pane is player list
    lobbyWidget.lobbyPane:animateToState("right_offscreen", lobbyWidget.fadeOutTime, true, true)

    CoD.Menu.goBack(lobbyWidget, client.controller)
end
...
CoD.Lobby.AddLivestreamButton = function(f27_arg0, f27_arg1, f27_arg2)
    if UIExpression.DvarBool(nil, "webm_encUiEnabled") == 1 then
        f27_arg0.body.liveStreamingButton = f27_arg0.body.buttonList:addButton(
            Engine.Localize("MENU_LIVESTREAMING_CAPS"), Engine.Localize("MENU_LIVESTREAMING_DESC"), f27_arg1)
...
CoD.Lobby.ToggleLivestream = function(f30_arg0, f30_arg1)
    ...
    if Engine.IsFeatureBanned(CoD.FEATURE_BAN_LIVE_STREAMING) then
        ...
        return
    elseif UIExpression.DvarBool(nil, "webm_encEliteRequired") == 1 and
        not Engine.IsPlayerEliteRegistered(f30_arg1.controller) then
...
CoD.Lobby.ConfirmLeaveGameLobby = function(f35_arg0, f35_arg1)
    ...
    if Engine.GameModeIsMode(CoD.GAMEMODE_LOCAL_SPLITSCREEN) == true or
        Engine.SessionModeIsMode(CoD.SESSIONMODE_SYSTEMLINK) == true then
        ...
        return
    elseif UIExpression.AloneInPartyIgnoreSplitscreen(f35_arg1.controller, 1) == 1 and
        UIExpression.InPrivateParty(f35_arg1.controller) == 1 then
        f35_local0.titleText = Engine.Localize("MENU_LEAVE_LOBBY_TITLE")
        table.insert(f35_local0.params, {
            leaveHandler = f35_arg1.leaveLobbyHandler,
            leaveEvent = "confirm_leave",
