...
CoD.OffhandIcons.new = function ( menu, controller )
	local self = LUI.UIElement.new( controller )
	self.type = menu
	self.setMaterialAndQuantity = CoD.OffhandIcons.SetMaterialAndQuantity
	self:registerEventHandler( "hud_update_offhand", CoD.OffhandIcons.UpdateOffhand )
	return self
end
...
CoD.OffhandIcons.SetMaterialAndQuantity = function ( f2_arg0, f2_arg1, f2_arg2 )
	...
	for f2_local6 = 1, f2_arg2, 1 do
		local self = f2_local0[f2_local6]
		if self ~= nil then
			self:beginAnimation( "default" )
			self:setAlpha( f2_local5 )
			self:setImage( f2_arg1 )
		else
			self = LUI.UIImage.new()
			self:setLeftRight( false, true, f2_local2 - f2_local1, f2_local2 )
			self:setTopBottom( false, true, -f2_local1, 0 )
			self:setRGB( CoD.HUDBaseColor.r, CoD.HUDBaseColor.g, CoD.HUDBaseColor.b )
			self:setAlpha( CoD.HUDAlphaFull * f2_local6 / f2_arg2 )
			self:setImage( f2_arg1 )
			f2_local0[f2_local6] = self
