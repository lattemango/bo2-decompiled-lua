...
CoD.MainMenu.ShowStoreButton = function(f3_arg0)
    if not CoD.isPC and not CoD.isWIIU and UIExpression.IsFFOTDFetched(f3_arg0) == 1 and
        Dvar.ui_inGameStoreVisible:get() == true and (CoD.isPS3 ~= true or CoD.isZombie ~= true) then
        return true
    else
        return false
    end
...
LUI.createMenu.MainMenu = function(f4_arg0)
    ...
    if CoD.isZombie == false then
        ...
        f4_local0:registerEventHandler("elite_registration_email_popup_requested",
    ...
    if not CoD.isPC and not Engine.IsBetaBuild() then
        if CoD.isWIIU then
            f4_local0.localButton = f4_local0.buttonList:addButton(
                Engine.Localize(CoD.MPZM("MENU_LOCAL_CAPS", "PLATFORM_UI_LOCAL_CAPS")), nil, 3)
        else
            f4_local0.localButton = f4_local0.buttonList:addButton(
                Engine.Localize(CoD.MPZM("MENU_LOCAL_CAPS", "ZMUI_LOCAL_CAPS")), nil, 3)
        end
        f4_local0.localButton:setActionEventName("open_local_match_lobby")
    end
    if CoD.isWIIU then
        f4_local0.controlsButton = f4_local0.buttonList:addButton(Engine.Localize("MENU_CONTROLLER_SETTINGS_CAPS"), nil,
            4)
    ...
    if f4_local7 then
        ...
        if CoD.useMouse then
            ...
            f4_local0.systemLinkFlyoutContainer.buttonList:registerEventHandler("leftmouseup_outside",
                CoD.MainMenu.FlyoutBack)
        end
        f4_local0.systemLinkFlyoutContainer.openSystemLinkButton =
            f4_local0.systemLinkFlyoutContainer.buttonList:addButton(f4_local11, nil, 1)
        f4_local0.systemLinkFlyoutContainer.openSystemLinkButton:setActionEventName("open_system_link_lobby")
        f4_local0.systemLinkFlyoutContainer.openServerBrowserButton =
            f4_local0.systemLinkFlyoutContainer.buttonList:addButton(f4_local13, nil, 1)
...
CoD.MainMenu.ShowDLC0Popup = function(f11_arg0)
    if CoD.isXBOX == true and Engine.IsContentAvailableByPakName("dlc0") == false and
        UIExpression.DvarBool(nil, "ui_isDLCPopupEnabled") == 1 and CoD.perController[f11_arg0].IsDLCPopupViewed == nil then
        return true
    else
        return false
    end
...
CoD.MainMenu.ShowSPReminderPopup = function(f13_arg0)
    if CoD.isPC then
        return false
    elseif Engine.ShouldShowSPReminder(f13_arg0) == true and Engine.OwnSeasonPass(f13_arg0) == true and
        CoD.MainMenu.AnyDLCMissing() == true then
        return true
    else
        return false
    end
...
CoD.MainMenu.ShowDSPPromotionPopup = function(f14_arg0)
    if Engine.ShouldShowDSPPromotion(f14_arg0) == true and Engine.OwnSeasonPass(f14_arg0) == false and
        Engine.OwnDLC1Only(f14_arg0) == true then
        return true
    else
        return false
    end
...
CoD.MainMenu.OpenMainLobbyRequested = function(f15_arg0, f15_arg1)
    ...
    elseif CoD.isZombie == false then
        if CoD.MainLobby.OnlinePlayAvailable(f15_arg0, f15_arg1) == 1 then
            ...
            elseif Engine.ShouldShowMOTD(f15_arg1.controller) ~= nil and Engine.ShouldShowMOTD(f15_arg1.controller) ==
                true then
...
CoD.MainMenu.ButtonPromptFriendsMenu = function(f36_arg0, f36_arg1)
    ...
    elseif UIExpression.IsSignedInToLive(controller) == 0 then
        ...
        f36_local0.anyControllerAllowed = true
        return
    elseif UIExpression.IsContentRatingAllowed(f36_arg1.controller) == 0 or UIExpression.IsAnyControllerMPRestricted() ==
        1 or not Engine.HasMPPrivileges(f36_arg1.controller) then
        local f36_local0 = f36_arg0:openPopup("Error", controller)
