...
CoD.LiveStream.CanPostOnTwitter = function ()
	...
	if UIExpression.DvarBool( nil, "twEnabled" ) == 1 and f2_local0 then
		return true
	else
		return false
	end
...
function LiveStream_UpdateCameraButtonState( f12_arg0 )
	if UIExpression.DvarBool( nil, "webm_encAllowCamera" ) == 0 then
		return 
	end
	local f12_local0 = false
	local f12_local1 = f12_arg0:getOwner()
	if UIExpression.DvarBool( nil, "webm_encIdentityVerificationRequired" ) == 0 then
		f12_local0 = true
	end
	if Dvar.webm_httpAuthMode:get() == "youtube" then
	...
	elseif not f12_local0 then
		f12_arg0.buttonList.visionCamera.hintText = Engine.Localize( "PLATFORM_TOGGLE_CAMERA_SMS_VERIFICATION_FAILED", UIExpression.DvarString( nil, "YouTube_verifyUrl" ) )
	else
...
local f0_local8 = function ( f17_arg0, f17_arg1 )
	...
	if f17_arg1.disabled == true then
		...
		if UIExpression.DvarBool( nil, "webm_encAllowCamera" ) == 1 then
...
LUI.createMenu.LiveStream = function ( f20_arg0, f20_arg1 )
	...
	if CoD.isPC then
		f20_local0.buttonList:setLeftRight( true, false, 0, 380 )
	end
	if (not (f20_arg1 == nil or not f20_arg1.isHost) or Dvar.r_stereo3DOn:get() or not Engine.CanLivestream() or not Dvar.hiDef:get()) and Engine.IsLivestreamEnabled() then
		Engine.LivestreamDisable( f20_arg0 )
	end
	...
	elseif not Dvar.hiDef:get() then
		f20_local0.buttonList.liveStreaming:disable()
		if CoD.isPC then
			f20_local0.buttonList.liveStreaming.hintText = Engine.Localize( "PLATFORM_LIVESTREAMING_DISABLED_FOR_SD" )
		else
			f20_local0.buttonList.liveStreaming.hintText = Engine.Localize( "MENU_LIVESTREAMING_DISABLED_FOR_SD" )
		end
	elseif 1 < UIExpression.GetUsedControllerCount() then
		f20_local0.buttonList.liveStreaming:disable()
	...
	if 1 == UIExpression.DvarBool( nil, "webm_encAllowCamera" ) then
		f20_local3 = nil
		if Engine.WebM_camera_IsEnabled() then
			f20_local3 = 1
		else
...
CoD.LiveStream.AddInGameStatusWidget = function ( f37_arg0, f37_arg1, f37_arg2 )
	if not Engine.IsLivestreamEnabled() or UIExpression.IsDemoMovieRendering() == 1 then
		return 
	end
...
local f0_local24 = function ( f51_arg0, f51_arg1 )
	...
	elseif f51_arg1.down == true then
		if f51_arg1.qualifier == nil then
			...
			return 
		elseif f51_arg1.qualifier == "mwheel" then
			if f51_arg1.button == "up" then
				Dvar.ui_scrollByRow:set( -1 )
			elseif f51_arg1.button == "down" then
				Dvar.ui_scrollByRow:set( 1 )
			end
		end
...
LUI.createMenu.LiveStreamTOS = function ( f53_arg0 )
	...
	if CoD.isPC then
		f53_local0.selectButton = CoD.ButtonPrompt.new( "primary", Engine.Localize( "MPUI_TOS_AGREE" ), f53_local0, "button_select_prompt" )
		f53_local0.selectButton:setPriority( -10 )
		f53_local0:addLeftButtonPrompt( f53_local0.selectButton )
	else
	...
	f53_local0.creditsContainer.border = CoD.Border.new( 1, 1, 1, 1, 0.1 )
