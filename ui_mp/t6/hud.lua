...
local f0_local0, f0_local1, f0_local2, f0_local3, f0_local4, f0_local5, f0_local6, f0_local7, f0_local8, f0_local9, f0_local10, f0_local11, f0_local12, f0_local13 = nil
local f0_local14 = 10
...
LUI.createMenu.HUD = function ( f2_arg0 )
	local f2_local0 = CoD.Menu.NewFromState( "HUD", {
		leftAnchor = true,
		rightAnchor = true,
		left = 0,
		right = 0,
...
local f0_local16 = function ( f8_arg0, f8_arg1 )
	local f8_local0 = LUI.roots
	local f8_local1 = "UIRoot"
	if f8_local0["UIRoot" .. f8_arg1.controller].ingameFriendsList then
...
function HUD_FirstSnapshot_Common( f17_arg0, f17_arg1 )
	...
	f17_arg0.scoreboardUpdateTimer = LUI.UITimer.new( 1000, {
...
function HUD_FirstSnapshot_Multiplayer( f18_arg0, f18_arg1 )
	...
	if not Engine.IsSplitscreen() then
		CoD.GameMessages.AddObituaryWindow( f18_arg0, {
			leftAnchor = true,
			rightAnchor = false,
			left = 13,
			right = 277,
	...
	CoD.GameMessages.BoldGameMessagesWindow( f18_arg0, {
		leftAnchor = false,
		rightAnchor = false,
		left = 0,
		right = 0,
	...
	if Engine.GameModeIsMode( CoD.GAMEMODE_LOCAL_SPLITSCREEN ) == false then
		CoD.LiveStream.AddInGameStatusWidget( f18_arg0, f18_arg1.controller, {
			leftAnchor = false,
			rightAnchor = true,
			left = -200,
			right = 0,
...
function HUD_FirstSnapshot_Zombie( menu, controller )
	...
	if CoD.Zombie.GAMETYPEGROUP_ZENCOUNTER == UIExpression.DvarString( nil, "ui_zm_gamemodegroup" ) then
		self:addElement( CoD.Hud3DScoreBoardZombie.new( {
			leftAnchor = true,
			rightAnchor = true,
			left = 0,
			right = 0,
...
function HUD_OpenScoreBoard( f23_arg0, f23_arg1 )
	if CoD.isZombie == true and CoD.InGameMenu.m_unpauseDisabled ~= nil and CoD.InGameMenu.m_unpauseDisabled[f23_arg1.controller + 1] ~= nil and CoD.InGameMenu.m_unpauseDisabled[f23_arg1.controller + 1] > 0 then
		return 
...
f0_local2 = function ( f31_arg0, f31_arg1 )
	if UIExpression.IsVisibilityBitSet( f31_arg1.controller, CoD.BIT_IN_KILLCAM ) == 1 then
		if f31_arg0.killcamHUD then
			...
			elseif f31_arg0.killcamHUD.isRoundEndKillcam ~= UIExpression.IsVisibilityBitSet( f31_arg1.controller, CoD.BIT_ROUND_END_KILLCAM ) then
				HUD_StopKillcamHud( f31_arg0, f31_arg1 )
			end
		end
...
f0_local5 = function ( f36_arg0, f36_arg1 )
	if f0_local19( f36_arg1.controller ) then
		if f36_arg0.locationSelectorMap == nil then
			...
			f36_arg0.locationSelectorMap = CoD.Compass.new( {
				leftAnchor = false,
				rightAnchor = false,
				left = -275,
				right = 275,
...
f0_local9 = function ( f40_arg0, f40_arg1 )
	...
	if f40_local0 == "TEAM_ALLIES" then
		...
		f40_local5 = UIExpression.ToUpper( nil, Engine.Localize( Engine.GetIString( f40_arg1.data[1], "CS_LOCALIZED_STRINGS" ), Dvar.g_TeamName_Allies:get() ) )
	elseif f40_local0 == "TEAM_AXIS" then
		...
		f40_local5 = UIExpression.ToUpper( nil, Engine.Localize( Engine.GetIString( f40_arg1.data[1], "CS_LOCALIZED_STRINGS" ), Dvar.g_TeamName_Axis:get() ) )
	else
		return 
	end
	...
	self.text = CoD.AdditiveTextOverlay.newWithText( f40_local5, "Condensed", f40_local2, f40_local3, f40_local4, {
		leftAnchor = true,
		rightAnchor = true,
		left = 0,
		right = 0,
	...
	self.imageGlow = CoD.AdditiveTextOverlay.new( f40_local6, f40_local6, f40_local2, f40_local3, f40_local4, {
		leftAnchor = false,
		rightAnchor = false,
		left = -f40_local6 / 2,
		right = f40_local6 / 2,
...
function HUD_IsFFA()
	if f0_local13 == nil then
		if CoD.isZombie == true then
			...
			local f44_local0
			if UIExpression.DvarString( nil, "ui_gametype" ) ~= "dm" and UIExpression.DvarString( nil, "ui_gametype" ) ~= "hcdm" and UIExpression.DvarString( nil, "ui_gametype" ) ~= "hack" and CoD.IsWagerMode() ~= true then
				f44_local0 = false
			else
				f44_local0 = true
			end
...
function HUD_Handle_ChooseClass_HotKey( f45_arg0, f45_arg1 )
	if UIExpression.Team( f45_arg1.controller, "name" ) ~= "TEAM_SPECTATOR" and CoD.IsWagerMode() == false and not (Engine.GetGametypeSetting( "disableClassSelection" ) == 1) then
		f0_local0( f45_arg0, {
